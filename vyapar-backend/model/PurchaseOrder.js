...
exports.ensureUser = async (connection, userId) => {
    ...
    // 2. Check whether the exact user exists
    ...
    // 4. Return the valid user ID
    return users[0].id;
};

// ENSURE SUPPLIER EXISTS
exports.ensureSupplier = async (connection, supplierId, userId, supplierName = null, supplierData = {}) => {
    if (!userId) {
        throw new Error('User ID is required');
    }
    ...
    if (supplierName && String(supplierName).trim()) {
        ...
        const [namedSuppliers] = await connection.query(
            `SELECT id FROM suppliers WHERE user_id = ? AND LOWER(supplier_name) = LOWER(?) LIMIT 1`,
            [userId, cleanName]
        );
        if (namedSuppliers.length > 0) {
            return namedSuppliers[0].id;
        }

        // Auto-create supplier with full details provided
        const [createResult] = await connection.query(
            `INSERT INTO suppliers (user_id, supplier_name, phone, email, gstin, address, city, state, pincode, status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                userId,
                cleanName,
                ...
                supplierData.gstin || null,
                supplierData.address || 'Default Address',
                supplierData.city || 'City',
                supplierData.state || 'State',
                supplierData.pincode || '000000',
                'active'
            ]
        );
    ...
    // Fallback: Use user's first available supplier
    const [userSuppliers] = await connection.query(
        `SELECT id FROM suppliers WHERE user_id = ? ORDER BY id ASC LIMIT 1`,
        [userId]
    );
    ...
    // Auto-create a default supplier for this user if none exists
    const [createResult] = await connection.query(
        `INSERT INTO suppliers (user_id, supplier_name, status) VALUES (?, ?, ?)`,
        [userId, 'General Supplier', 'active']
    );
...
exports.ensureProduct = async (connection, productId, userId, productName = null, purchasePrice = 0, gstRate = 18) => {
    if (!userId) {
        throw new Error('User ID is required');
    }
    ...
    if (productName && String(productName).trim()) {
        ...
        const [namedProducts] = await connection.query(
            `SELECT id FROM products WHERE business_id = ? AND LOWER(product_name) = LOWER(?) LIMIT 1`,
            [userId, cleanName]
        );
        if (namedProducts.length > 0) {
            return namedProducts[0].id;
        }

        // Auto-create product with the specified product name
        const [createResult] = await connection.query(
            `INSERT INTO products (business_id, product_name, purchase_price, gst_rate) VALUES (?, ?, ?, ?)`,
            [userId, cleanName, purchasePrice || 0, gstRate || 18]
        );
        return createResult.insertId;
    }

    // Fallback: Use user's first available product
    const [userProducts] = await connection.query(
        `SELECT id FROM products WHERE business_id = ? ORDER BY id ASC LIMIT 1`,
        [userId]
    );

    if (userProducts.length > 0) {
        return userProducts[0].id;
    }

    // Auto-create a default product for this business if none exists
    const [createResult] = await connection.query(
        `INSERT INTO products (business_id, product_name, purchase_price, gst_rate) VALUES (?, ?, ?, ?)`,
        [userId, 'General Product', 0, 18]
    );
...
exports.getPurchaseOrderById = async (connection, purchaseOrderId, userId) => {
    ...
    const purchaseOrder = {
        ...
        supplier_gstin: rows[0].supplier_gstin,
        supplier_address: rows[0].supplier_address,
        supplier_city: rows[0].supplier_city,
        supplier_state: rows[0].supplier_state,
        supplier_pincode: rows[0].supplier_pincode,
...
exports.updatePurchaseOrder = async (
    ...
    // Update document only if a new document was uploaded or explicitly removed
    if (hasNewDocument) {
        updateSql += `
            , document_name = ?
            , document_type = ?
    ...
    for (const item of items) {
        ...
        const validProductId =
            await exports.ensureProduct(
                ...
                item.purchase_price,
                item.tax_rate || item.item_tax_rate
            );
        ...
        const itemDiscount = Number(
            item.discount !== undefined
                ? item.discount
                : (item.item_discount !== undefined ? ((amount * Number(item.item_discount)) / 100) : 0)
        );
        ...
        const taxableAmount = Math.max(0, amount - itemDiscount);

        const itemTaxAmount =
            item.tax_amount !== undefined && item.tax_amount !== null
                ? Number(item.tax_amount)
                : item.item_tax_amount !== undefined && item.item_tax_amount !== null
        ...
        const itemTotal =
            item.total_amount !== undefined && item.total_amount !== null
                ...
                : item.item_total_amount !== undefined && item.item_total_amount !== null
