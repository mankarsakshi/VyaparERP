...
exports.createPurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        if ((!supplier_id && !supplier_name) || !po_date) {
            return res.status(400).json({
                success: false,
                message: 'supplier_id (or supplier_name) and po_date are required'
            });
        }
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (
                (!item.product_id && !item.product_name && !item.productName) ||
                item.quantity === undefined || item.quantity === null ||
                item.purchase_price === undefined || item.purchase_price === null
            ) {
                return res.status(400).json({
                    success: false,
                    message: `Item at index ${i} is missing product_id/product_name, quantity, or purchase_price`
                });
            }
        }
        ...
        const validSupplierId = await PurchaseOrder.ensureSupplier(
            ...
            supplier_name,
            {
                phone: supplier_phone || phone,
                email: supplier_email || email,
                gstin: supplier_gstin || gstin,
...
exports.updatePurchaseOrder = async (req, res) => {
    ...
    try {
        ...
        if (!allowedStatuses.includes(newStatus)) {
            ...
            return res.status(400).json({
                success: false,
                message: 'Invalid status. Allowed values are Pending, Approved, Received, and Cancelled'
            });
        }
        ...
        const validSupplierId = await PurchaseOrder.ensureSupplier(
            ...
            {
                phone: req.body?.supplier_phone || req.body?.phone,
                email: req.body?.supplier_email || req.body?.email,
                gstin: req.body?.supplier_gstin || req.body?.gstin,
                address: req.body?.supplier_address || req.body?.address
            }
        );

        // 8. Update Purchase Order
        const removeDocument = req.body?.remove_document === true || req.body?.remove_document === 'true' || req.body?.delete_document === 'true';
